...
const saveEmailInFile = async (data) => {
	try {
		...
		mailComposer.pipe(fs.createWriteStream(`${pathFolder}/${new Date().getTime()}.eml`))
...
const send = async (data) => {
	try {
		...
		if (config.app.isDev) {
			saveEmailInFile(data)
			return { status: true, message: "Message has been sent. Please check your mail." }
		}
		...
		// throw new AppError(err.message)
		return { status: false, message: "Message send error. Please check your configuration." }
	}
}
